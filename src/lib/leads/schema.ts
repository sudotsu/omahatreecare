...
export function isQualifiedLead(lead: LeadInput) {
  ...
  const zip = lead.address.match(/\b(\d{5})(?:-\d{4})?\b/)?.[1];
  const hasNebraskaZip = zip ? Number(zip) >= 68_001 && Number(zip) <= 69_367 : false;
  const hasMetroCity = /\b(Omaha|Bellevue|Papillion|Ralston|Gretna|Elkhorn|Bennington|Council Bluffs)\b/i.test(lead.address);
  const nearby = !lead.address || hasMetroCity || hasNebraskaZip;
