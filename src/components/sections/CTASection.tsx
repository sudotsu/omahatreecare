    ...
    <Section spacing="lg" className="bg-neutral-50">
      <Container size="lg">
        <div className={`${styleClasses.container} rounded-xl shadow-xl p-8 md:p-12 text-center`}>
          <h2 className={`text-3xl md:text-4xl font-bold mb-4 ${styleClasses.title}`}>{title}</h2>
          ...
          {note && <p className={`mt-6 text-sm ${styleClasses.note}`}>{note}</p>}
        </div>
      </Container>
    </Section>
  );
...
export const QuickPhoneCTA: React.FC<{
  ...
  return (
    <CTASection
      ...
      note={
        variant === "emergency" ? "24/7 Emergency Service Available" : "Available daily 7am - 9pm"
      }
    />
  );
};
