    ...
    <Section spacing="lg" className={bgClasses}>
      <Container size="lg">
        ...
        <div className="mb-8 max-w-3xl">
          <h2 className={`text-3xl md:text-4xl font-bold mb-4 ${titleClasses}`}>{title}</h2>
          {description && <p className={`text-lg leading-relaxed ${descClasses}`}>{description}</p>}
        </div>
            ...
            return (
              <div key={index} className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className={`font-bold text-lg mb-2 ${itemTitleClasses}`}>{item.title}</h3>
                  <p className={`leading-relaxed ${itemDescClasses}`}>{item.description}</p>
                </div>
              </div>
            );
          })}
