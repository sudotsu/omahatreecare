  ...
  return (
    <div className={`${bgClasses} py-12 md:py-16`}>
      <Container size="xl">
            ...
            <ol className="flex items-center gap-2 flex-wrap">
              {breadcrumbs.map((crumb, index) => (
                <li key={crumb.href} className="flex items-center gap-2">
                  <Link
                    href={crumb.href}
                    className={`${breadcrumbClasses} transition-colors`}
                  >
                    {crumb.label}
                  </Link>
        ...
        {/* Optional Badge */}
        {badge && (
          <div className={`inline-block border px-4 py-2 rounded-full text-sm font-semibold mb-4 ${badgeStyles}`}>
            {badge.text}
          </div>
        )}
        ...
        {/* Eyebrow */}
        {eyebrow && (
          <p className={`text-sm font-semibold uppercase tracking-wider mb-3 ${textClasses} opacity-80`}>
            {eyebrow}
          </p>
        )}

        {/* Title (H1) */}
        <h1 className={`text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-4 ${textClasses}`}>
          {title}
        </h1>
