...
import { submitLeadForm, type FormSubmissionData } from "@/lib/emailjs";
...
export default function ContactForm() {
  ...
  const onSubmit = async (data: ContactFormData) => {
    ...
    try {
      ...
      setSubmitStatus("error");
      setErrorMessage("Unable to send your request. Please call us directly.");
    } finally {
    ...
    return (
      <Card variant="feature" className="text-center py-12 space-y-6">
        ...
        <div className="space-y-2">
          <h3 className="text-3xl font-black text-content-heading uppercase italic tracking-tight">
            Estimate Requested
          </h3>
          <p className="text-content-body font-medium max-w-sm mx-auto">
            We&apos;ve received your details. One of our arborists will call you shortly to schedule
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
      {submitStatus === "error" && (
        <Alert variant="error">
          ...
          <p className="text-sm">
            {errorMessage || `Please call us directly at `}
            <a href={`tel:${CONTACT.phoneRaw}`} className="underline">
              {CONTACT.phone}
            </a>
            {errorMessage ? "" : " for immediate assistance."}
          </p>
        </Alert>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label
            htmlFor="user_name"
            className="text-xs font-black uppercase tracking-widest text-content-muted ml-1"
          >
            Full Name *
          </label>
          <Input
          ...
          />
          {errors.user_name && (
            <p className="text-xs text-alert-600 font-bold ml-1">{errors.user_name.message}</p>
          )}
        </div>
        <div className="space-y-2">
          <label
          ...
          />
          {errors.user_phone && (
            <p className="text-xs text-alert-600 font-bold ml-1">{errors.user_phone.message}</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        />
        {errors.user_email && (
          <p className="text-xs text-alert-600 font-bold ml-1">{errors.user_email.message}</p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        />
        {errors.address && (
          <p className="text-xs text-alert-600 font-bold ml-1">{errors.address.message}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex items-end">
          <div className="bg-primary-50 dark:bg-primary-900/20 p-4 rounded-xl border border-primary-100 dark:border-primary-800 flex items-center gap-3 w-full">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-tighter text-primary-700/70">
                Urgent Storm Damage?
              </p>
              <a
                href={`tel:${CONTACT.phoneRaw}`}
                className="text-sm font-black text-primary-900 dark:text-primary-100 hover:underline"
              >
                {CONTACT.phone}
              </a>
            </div>
