...
export const Select = React.forwardRef<HTMLSelectElement, SelectProps>(
  ({ error = false, children, className = "", ...props }, ref) => {
    const baseStyles =
      "w-full py-3 px-4 text-base leading-normal rounded-md transition-all duration-default ease-smooth focus:outline-none focus:ring-2 appearance-none bg-white";
    ...
    return (
      <div className="relative">
        <select ref={ref} className={`${baseStyles} ${stateStyles} ${className}`} {...props}>
          {children}
        </select>
