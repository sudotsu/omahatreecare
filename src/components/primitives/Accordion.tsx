...
export const AccordionItem: React.FC<AccordionItemProps> = ({
  ...
  return (
    <div className="border-b border-neutral-200 last:border-0">
      ...
      >
        <div className="text-base text-neutral-600 leading-relaxed pr-8">
          {children}
        </div>
      </div>
    </div>
...
export const Accordion: React.FC<{ children: React.ReactNode, className?: string }> = ({
  ...
  return (
    <div className={`w-full rounded-lg border border-neutral-200 bg-white px-6 shadow-sm ${className}`}>
      {children}
    </div>
  );
};
